package interactive_maxflow

import (
	"testing"

	"github.com/ivasnev/FinFlow/ff-common/optimizers"
	"github.com/ivasnev/FinFlow/ff-common/optimizers/maxflow/dinic"
	"github.com/ivasnev/FinFlow/ff-common/optimizers/tests/testutil"
	"github.com/ivasnev/FinFlow/ff-common/optimizers/utils/validator"
)

var _ optimizers.Optimizer = (*Optimizer)(nil)

func TestFlowPerEdge_DirectSuccess(t *testing.T) {
	opt := New(dinic.Solver{})
	v := validator.NewValidator(validator.WithAllChecks())
	debts := testutil.DebtsDirectSimple()
	result, err := opt.Optimize(debts)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	report := v.Validate(debts, result)
	if !report.Valid {
		t.Fatalf("expected valid result: %+v", report.Violations)
	}
}

func TestFlowPerEdge_IntermediateSuccess(t *testing.T) {
	opt := New(dinic.Solver{})
	v := validator.NewValidator(validator.WithAllChecks())
	debts := testutil.DebtsNeedsIntermediate()
	result, err := opt.Optimize(debts)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	report := v.Validate(debts, result)
	if !report.Valid {
		t.Fatalf("expected valid result: %+v", report.Violations)
	}
}

func TestFlowPerEdge_ComplexGraph(t *testing.T) {
	opt := New(dinic.Solver{})
	v := validator.NewValidator(validator.WithAllChecks())
	debts := testutil.DebtsComplexGraph()
	result, err := opt.Optimize(debts)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	report := v.Validate(debts, result)
	if !report.Valid {
		t.Fatalf("expected valid result: %+v", report.Violations)
	}
}

func TestFlowPerEdge_Empty(t *testing.T) {
	opt := New(dinic.Solver{})
	result, err := opt.Optimize(testutil.DebtsEmpty())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result) != 0 {
		t.Fatalf("expected empty result, got %v", result)
	}
}
